...
- 完整数据：公开重建的五档难度各 100 题，共 500 题；固定种子 42，每题 200 次模拟；
  2026-07-28 08:43:57 UTC 按要求优雅暂停，已完成 350 个最终标签和
  94,450 / 100,500 条搜索路径（94.0%）；十个 append-only cache 的末行均可解析；
  10:39:13 UTC 确认两张 A800 空闲后从原 cache 恢复，只续跑 DM-4 shard 1 和 DM-5 的
  两个 shard，已完成的七个 shard 自动跳过
